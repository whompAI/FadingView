...
return true;`);
  ...
  function renderWatchlist() {
    ...
    if (!container) return;
    container.innerHTML = state.watchlist.map(
      (sym) => `
    <div class="watchlist-item" data-symbol="${sym}" style="display:flex;justify-content:space-between;padding:8px;cursor:pointer;border-radius:4px;margin-bottom:2px;background:${sym === state.selected ? "#238636" : "transparent"};hover{background:#21262d;}">
      ...
      <button onclick="window._FV_remove('${sym}', event)" style="background:none;border:none;color:#f85149;cursor:pointer;padding:0 4px;font-size:16px;line-height:1;">\xD7</button>
    </div>
  `
    ).join("");
  }
